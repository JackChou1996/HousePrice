...
lst = list(  set( tag.columns )-set(['id','gdr_cd','mrg_situ_cd','acdm_deg_cd','deg_cd','edu_deg_cd',]))
dic = dict(zip(  lst   ,['float64']*len(lst))     )
tag = tag.astype(dic)
tag.dtypes
...
#统计学分析
tag.astype(np.float)
tag.astypes = np.float
tag.dtypes
train_tag_corr = train_tag.corr(method = 'spearman')
tc1 = tag.columns
tag_corr = tag.corr()
tc2 = tag_corr
set(tc1).difference(set(tc2))
tag_describe = tag.describe()
    ...
    def init(train_beh_path,train_tag_path,train_trd_path,pred_beh_path,pred_tag_path,pred_trd_path,):
        ...
        tag['gdr_cd'] = label_hot(tag['gdr_cd'])
        tag['edu_deg_cd'] = label_hot(tag['edu_deg_cd'])
        tag['acdm_deg_cd'] = label_hot(tag['acdm_deg_cd'])
        tag['deg_cd'] = label_hot(tag['deg_cd'])
        tag['mrg_situ_cd'] = label_hot(tag['mrg_situ_cd'])
...
#transaction['sz_amt'] = sz_amt

#transaction['flag'].corr(code1_mean)
    ...
    def beh_map(user , beh , prt_dt='2019-07-01'):
        ...
        page_no = beh.groupby([beh.index,beh.page_no]).size().unstack() #成功把multiindex 做成二维矩阵

        page_tm = beh.page_tm.groupby(beh.index).size() #.unstack(fill_value=0) #合并user和新造的特征
        page_no_mean =  (page_no.T/page_no.sum(axis=1)).T    #page_no/page_no.sum(axis=1)
        ...
        user = pd.merge(user, page_no_mean, how='left', on='id')
...
#temp = pd.merge(user['flag'], page_no, how='left', on='id')
#temp = pd.merge(temp, page_tm, how='left', on='id')
#temp = pd.merge(user['flag'] , hour, how='left', on='id')
#temp = pd.merge(temp , hour_max, how='left', on='id')
#temp = pd.merge(temp , hour_min, how='left', on='id')
...
def data_prepare(train, oversampling='s', test_size=0.2, seed=1000):
    ...
    if oversampling == 's':
        ...
        train_x, test_x, train_y, test_y = train_test_split(train.loc[:, ~ (data.columns == 'flag')], train.flag,
                                                            test_size=test_size, stratify=list(train.flag),
                                                            random_state=seed)
...
result = pd.DataFrame( model.predict_proba(pred,),columns=['0','1'],index=pred.index  )
result['id'] = result.index
result[['1']].sort_values(by='id',ascending= True).to_csv( 'data/result.txt', header=None, index=True, sep="\t")
